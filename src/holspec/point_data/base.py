...
class PointData:
    ...
    @classmethod
    def load(
        cls, filepath: str | Path, group: str | None = None, validate_hash: bool = True
    ) -> "PointData":
        """
        Load from HDF5 file.
        ...
        if validate_hash:
            if "content_hash" not in attributes:
                print(
                    f"Warning: No 'content_hash' in attributes for {filepath}, skipping validation"
                )
            else:
                stored_hash = str(attributes["content_hash"])
                computed_hash = compute_content_hash(
                    data_array, length=len(stored_hash)
                )
    ...
    @classmethod
    def from_config(
        cls, config: dict, noise_config: dict | None = None, seed: int = 42
    ) -> "PointData":
        """
        ...
        if noise_config is not None:
            ...
            metadata = {"config": config, "noise_config": noise_config, "seed": seed}
        else:
    ...
    def __repr__(self) -> str:
        ...
        if self.has_positions:
            return (
                f"PointData(N={self.num_points}, d={self.dimension}, "
                f"hash={self.content_hash})"
            )
        else:
            return (
                f"PointData(N={self.num_points}, distances_only, "
                f"hash={self.content_hash})"
            )
