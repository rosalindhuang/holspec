...
class HodgeLaplacianSpectra:
    ...
    compute_eigenvectors : bool or list of (int, str) tuples, default=False
        ...
        (k, component) tuples computes eigenvectors only for those pairs.
    ...
    def __init__(
        ...
        # Normalize compute_eigenvectors to frozenset for uniform lookup
        if isinstance(compute_eigenvectors, bool):
            ...
            # List of (k, component) pairs — validate entries
            for k, comp in compute_eigenvectors:
                if k not in range(hl.max_dim + 1):
                    raise ValueError(
                        f"Invalid degree k={k} in compute_eigenvectors. "
                        f"Valid range: 0 <= k <= {hl.max_dim}."
                    )
                if comp not in LAPLACIAN_COMPONENT_NAMES:
                    raise ValueError(
                        f"Unknown component '{comp}' in compute_eigenvectors. "
                        f"Valid components: {LAPLACIAN_COMPONENT_NAMES}."
                    )
            self._eigenvector_keys = frozenset(
                (k, comp) for k, comp in compute_eigenvectors
            )
    ...
    def compute_eigenvectors(self) -> bool | list:
        ...
        Returns the original value passed at construction: True, False,
        or a list of (int, str) tuples.
        """
